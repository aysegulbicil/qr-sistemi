<div class="empty-state">
    ...
    <?php if (! empty($actionUrl)): ?>
        <?php
        $emAttrs = '';
        if (! empty($actionModal)) {
            $emAttrs = ' data-modal data-modal-title="' . esc($actionTitle ?? $actionLabel ?? 'Ekle', 'attr') . '"';
            if (! empty($actionSize)) {
                $emAttrs .= ' data-modal-size="' . esc($actionSize, 'attr') . '"';
            }
        }
        ?>
        <a class="btn btn-primary btn-sm" href="<?= esc($actionUrl) ?>"<?= $emAttrs ?>><?= esc($actionLabel ?? 'Ekle') ?></a>
    <?php endif; ?>
</div>
